...
void litehtml::tooltips::draw_window(BOOL clr)
{
	...
	m_bg_cache.draw(m_cr, &m_layout, m_hWnd, m_alpha);
	...
	{
		double sb_ratio = (double) m_layout.content_height / (double) m_html->height();
		double sb_height = m_layout.content_height * sb_ratio;
		double sb_top = m_layout.content_y + m_top * sb_ratio;

		rounded_rect(m_cr, m_layout.content_x + m_layout.content_width + 3, (int) sb_top, 4, (int) sb_height, 2, 0);
...
void litehtml::tooltips::update_tool(unsigned int id, bool re_render, bool redraw_only)
{
	if(IsWindow(m_hWnd))
	{
		if(redraw_only)
		{
			PostMessage(m_hWnd, WM_REDRAW_TIP, (WPARAM) id, 0);
		} else
		{
			PostMessage(m_hWnd, WM_UPDATE_TIP, (WPARAM) id, (LPARAM) re_render);
		}
	}
}

bool litehtml::tooltips::is_tool_interactive( unsigned int id )
{
	tool::map::iterator ti = m_tools.find(id);
	if(ti != m_tools.end())
	{
		if(ti->second.options & tool_opt_interactive)
		{
			return true;
		}
	}
...
void litehtml::tooltips::hide_current_tool()
{
	if(!is_tool_interactive(m_show_tool))
	{
		hide();
	} else
	{
		if(!m_hide_timer_active)
		{
			m_hide_timer_active = true;
			SetTimer(m_hWnd, TIMER_HIDE_TIP, m_hide_time_int, NULL);
		}
	}
}

void litehtml::tooltips::on_mouse_over_tool( int x, int y )
{
	if(!m_disabled)
	{
		unsigned int over_id = find_tool(x, y);
		if(over_id != m_last_shown_tool)
		{
			m_last_shown_tool = 0;
		}

		if(over_id != m_show_tool)
		{
			hide_current_tool();
		}

		if(!m_show_tool && !m_last_shown_tool || m_hide_timer_active)
		{
			start_hover_tracking(true);
		}
	}
}

void litehtml::tooltips::on_mouse_hover_tool( int x, int y )
{
	m_mouse_hover_on = false;
	if(!m_disabled)
	{
...
void litehtml::tooltips::update_cursor()
{
...
void litehtml::tooltips::draw_borders( litehtml::uint_ptr hdc, const litehtml::css_borders& borders, const litehtml::position& draw_pos, bool root )
{
	if(root)
	{
		if(borders.top.width.val() != 0 && borders.top.style > litehtml::border_style_hidden)
		{
			...
			m_bg_cache.create_tip_path(cr, (int) borders.top.width.val());
			cairo_set_line_width(cr, borders.top.width.val());
			cairo_stroke(cr);
	...
	{
		cairo_container::draw_borders(hdc, borders, draw_pos, root);
	}
}
...
void litehtml::tooltips_bg_cache::create_tip_path( cairo_t* cr, int line_width )
{
	switch(m_layout.style)
	{
	case tips_style_rounded:
		tooltips::rounded_rect(cr, 0, 0, m_dib.width() - 5, m_dib.height() - 5, m_layout.radius, line_width);
		break;
	case tips_style_baloon:
		tooltips::baloon(cr, 0, 0, m_dib.width() - 5, m_dib.height() - 5, m_layout.anchor_x - m_layout.x, m_layout.anchor_y - m_layout.y, m_layout.align, m_layout.radius, line_width);
		break;
	default:
