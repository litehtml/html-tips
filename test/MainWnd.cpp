...
{
	{3,	{10, 10,	110, 110}, litehtml::tool_opt_align_left,	L"<h1><span style=\"color: red\">Lite HTML</span> engine</h1><hr /><p>The <b>Lite HTML</b> engine is created for embedding into <i>applications</i>, to show the HTML code. Lite HTML supports most of the CSS2/CSS3 standards. The engine is written on C++ with STL (<b>MS Visual Studio 2008</b>) and was tested on Windows platform only.</p><h2>Embedding Lite HTML</h2><p>Firstly, the Lite HTML don't have the code for draw anything. You are free to use any draw engine. We've included the win32_container class as example to show how to implement the draw code.</p>" },
	{2,	{10, 120,	110, 220}, litehtml::tool_opt_ask_text | litehtml::tool_opt_align_left,	L"Simple text" },
	{1,	{300, 300,	350, 350}, litehtml::tool_opt_ask_text | litehtml::tool_opt_align_top | litehtml::tool_opt_interactive,	L"<h1><span style=\"color: red\">Lite HTML</span> engine</h1><hr /><p>The <b>Lite HTML</b> engine is created for embedding into <i>applications</i>, to show the HTML code. Lite HTML supports most of the CSS2/CSS3 standards. The engine is written on C++ with STL (<b>MS Visual Studio 2008</b>) and was tested on Windows platform only.</p><h2>Embedding Lite HTML</h2><p>Firstly, the Lite HTML don't have the code for draw anything. You are free to use any draw engine. We've included the win32_container class as example to show how to implement the draw code.</p>" },
	{0,	{0, 0, 0, 0},	NULL },
};
...
LRESULT CMainWnd::OnMessage( HWND hWnd, UINT uMessage, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_TIMER:
		m_counter++;
		m_tips.update_tool(2, false, false);
		return 0;
	}
...
void CMainWnd::ttcb_get_text( unsigned int id, std::wstring& text, std::wstring& css_text )
{
	switch(id)
	{
	case 1:
		{
			...
			StringCchCat(path, MAX_PATH, L"tip1.html");
			LPWSTR html = load_text_file(path, true, L"UTF-8");
			if(html)
			{
				text = html;
				delete html;
...
CTxDIB* CMainWnd::ttcb_get_image( unsigned int id, LPCWSTR url, bool redraw_on_ready )
{
	return NULL;
}

bool CMainWnd::ttcb_on_anchor_click( unsigned int id, const std::wstring& url )
{
	ShellExecute(m_hWnd, NULL, url.c_str(), NULL, NULL, SW_SHOW);
